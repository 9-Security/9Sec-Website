...
// Robust Storage Helper to prevent crashes when blocked by Tracking Prevention
const storage = {
    get: (key) => {
...
async function loadInsights() {
    ...
    if (!j || !j.ok) {
        if (j.status !== 401) {
            el.innerHTML = `<div class="card" style="text-align:center; padding: 3rem;"><p style="color:var(--text-secondary)">[ERROR] Error synchronizing telemetry.</p></div>`;
        }
        return;
    }
...
async function runDirectDownload(envId) {
    ...
    if (j && j.ok) { window.open(API_BASE + "/api/governance/download/" + j.data.upload_session_id + "?token=" + encodeURIComponent(j.data.token), "_blank"); }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tabEnvs) tabEnvs.addEventListener('click', () => switchDashboardTab("Envs"));
    ...
    // Auto-login check: Single entry point
    (async () => {
        const authed = await loadMe();
        if (authed) {
            showDashboard();
        } else {
            // Ensure login overlay is visible if check fails
            const overlay = document.getElementById("gov-login-overlay");
            if (overlay) overlay.style.display = "flex";
        }
    })();
